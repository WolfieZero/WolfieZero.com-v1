...
foreach ($_COOKIE as $n => $v) {
    if (substr($n, 0, 20) == 'wordpress_logged_in_') {
        ...
        $cache_stop = true;
        return false;
    } else if (false && substr($n, 0, 14) == 'comment_author') {
        $cache_stop = true;
        return false;
    }
}
...
$lc_file = '/var/www/wolfiezero.com/public/content/cache/lite-cache' . '/' . strtolower($_SERVER['HTTP_HOST']) . $lc_uri . '/index' . $lc_group . '.html';
if (!is_file($lc_file))
    return false;

$lc_file_time = filemtime($lc_file);

if (12 > 0 && $lc_file_time < time() - (12 * 3600))
    return false;
...
if (0 == 0 && isset($_SERVER['HTTP_ACCEPT_ENCODING']) && strpos($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip') !== false) {
...
// Do not detect
    ...
    if (defined('IS_PHONE'))
        return IS_PHONE;
    return preg_match('/(up.browser|up.link|mmp|symbian|smartphone|midp|wap|phone|iphone|ipad|ipod|android|xoom)/i', strtolower($_SERVER['HTTP_USER_AGENT']));
}
